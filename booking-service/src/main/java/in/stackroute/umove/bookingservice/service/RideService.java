...
public class RideService implements RideServiceInterface {
    ...
    public Ride startRide(ObjectId rideId, String registrationNo) {
        ...
        if(ride == null) {
            throw new RideNotFoundException("Ride is not found for the rideId = "+rideId);
        }
        else {
    ...
    //Function to display ride details for the user
    @Override
    public Ride getRideDetails(ObjectId rideId) {
        Ride ride = rideRepo.findBy_id(rideId);
        if(ride == null) {
            throw new RideNotFoundException("Ride is not found for the rideId = "+rideId);
        }
        else {
            if (ride.getStatus().equalsIgnoreCase("endRideRequest")) {
                return ride;
            }
        }
        return ride;
